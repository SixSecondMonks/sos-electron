...
function Kinect2(options) {
  ...
  let location = ['kinect2', 'lib', 'NodeKinect2.dll'].join(path.sep);
  console.log('location is', location);

  let dllProperties = {
    assemblyFile: __dirname + location,
  ...
  this.edge.replay = edge.func(_.assign({ methodName: 'Replay' }, dllProperties));
...
Kinect2.prototype.replay = function(filePath) {
  return this.edge.replay({
    filePath: filePath
  }, true);
};
